...
if __name__ == '__main__':
    ...
    if not os.path.exists(settings.JSON_FILE_VER3):
        json_file = open(settings.JSON_FILE_VER3, "a+")
        json_file.close()
        iteration = 0
        ...
        json_file = open(settings.JSON_FILE_VER3, "r+")
    ...
    # 开始单次时间片实验
    node_num = node_num - 1
    json_file_name = settings.JSON_FILE_VER3
    
    pool = multiprocessing.Pool(5)

    experiment_iteration_max = 1000
