...
if __name__ == '__main__':
    ...
    # no from 9 to 13
    pickle_file = Path(load_experiment_median_from_pickle(11))
    ...
    if not os.path.exists(settings.JSON_FILE_VER2_11):
        json_file = open(settings.JSON_FILE_VER2_11, "a+")
        json_file.close()
        iteration = 0
        ...
        json_file = open(settings.JSON_FILE_VER2_11, "r+")
    ...
    # 开始单次时间片实验
    node_num = node_num - 3
    json_file_name = settings.JSON_FILE_VER2_11
    result_file_name = settings.RESULT_FILE_VER2_11
