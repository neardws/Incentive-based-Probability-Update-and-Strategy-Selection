...
from algorithm.IPUSS_ver2 import compute_potential_value, compute_probability_update_value, \
    ...
    random_channel_fading_gain, weighted_choice
...
def write_result(json_file_name, dict_data):
...
if __name__ == '__main__':
    ...
    # no from 9 to 13
    pickle_file = Path(load_experiment_median_from_pickle(13))
    ...
    if not os.path.exists(settings.JSON_FILE_VER2_13):
        json_file = open(settings.JSON_FILE_VER2_13, "a+")
        json_file.close()
        iteration = 0
        ...
        json_file = open(settings.JSON_FILE_VER2_13, "r+")
    ...
    # 开始单次时间片实验
    node_num = node_num - 1
    json_file_name = settings.JSON_FILE_VER2_13
    result_file_name = settings.RESULT_FILE_VER2_13
        ...
        *****************************************************************************************************************************************
        ...
        if iteration < 1000:
            ...
            if iteration % 100 == 0:
                ...
                pool.apply_async(write_result, args=(result_file_name, json_dict))
                print_to_console(str(iteration) + " 写入RESULT进程结束")
